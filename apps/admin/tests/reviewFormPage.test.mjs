...
test('review form has one fixed selector and two conditional field branches', async () => {
  ...
  assert.match(formSource, /name="title"/)
  assert.match(formSource, /rows=\{1\}/)
  assert.match(formSource, /review-form__title-textarea/)
  assert.match(formSource, /인터뷰 고객사\(의뢰처\)/)
  assert.match(formSource, /의뢰하신 제품/)
  assert.match(formSource, /name="requestedProduct"/)
  assert.match(formSource, /options=\{reviewRequestedProductOptions\}/)
  assert.match(formSource, /isReviewRequestedProduct/)
